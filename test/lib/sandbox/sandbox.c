...
uint64_t _smalloc(uint64_t size) {
    return kexecute2(GETOFFSET(smalloc), size, 0, 0, 0, 0, 0, 0);
}
...
uint64_t extension_create_file(const char* path, uint64_t nextptr) {
    ...
    if (ext_p && ks) {
        ...
        kwriteOwO(ext_p, &ext, sizeof(ext));
    } else {
        // XXX oh no a leak
    }
...
uint64_t make_ext_hdr(const char* key, uint64_t ext_lst) {
    ...
    if (khdr) {
        ...
        kwriteOwO(khdr, &hdr, sizeof(hdr));
        kwriteOwO(khdr + offsetof(struct extension_hdr, desc), key, strlen(key) + 1);
    }
...
void extension_add(uint64_t ext, uint64_t sb, const char* desc) {
    ...
    uint64_t ext_table = ReadKernel64(sb + sizeof(void *));
    uint64_t insert_at_p = ext_table + slot * sizeof(void*);
    uint64_t insert_at = ReadKernel64(insert_at_p);
...
int has_file_extension(uint64_t sb, const char* path) {
    ...
    if (insert_at != 0) {
        uint64_t ext_lst = ReadKernel64(insert_at + offsetof(struct extension_hdr, ext_lst));
        ...
        while (ext_lst != 0) {
            ...
            uint64_t data_len = ReadKernel64(ext_lst + offsetof(struct extension, data_len));
            if (data_len == plen) {
                uint64_t data = ReadKernel64(ext_lst + offsetof(struct extension, data));
