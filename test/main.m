...
struct __attribute__((__packed__)) JAILBREAKD_BYPASS_CODESIGN_FILE {
    uint8_t Command;
    char exec[1024];
};
...
int runserver() {
    ...
    kernel_slide = kernel_base - KADD_SEARCH;
    NSLog(@"[jelbrekd] slide: 0x%016llx", kernel_slide);
    init_kexecute();
    ...
    NSLog(@"[jelbrekd] Running server...");
    int sockfd = socket(AF_INET, SOCK_DGRAM, 0);
    if (sockfd < 0)
        NSLog(@"[jelbrekd] Error opening socket");
    int optval = 1;
    ...
    if (server == NULL) {
        NSLog(@"[jelbrekd] ERROR, no such host as %s", hostname);
        exit(0);
    }
    ...
    if (bind(sockfd, (struct sockaddr *)&serveraddr, sizeof(serveraddr)) < 0){
        NSLog(@"[jelbrekd] Error binding...");
        term_kexecute();
        exit(-1);
    }
    
    NSLog(@"[jelbrekd] Server running!");
